...
module.exports = async (sock, from, _, msg) => {
    if (!from.endsWith('@g.us')) {
        return sock.sendMessage(from, { text: '❌ `!promote` can only be used in group chats.' });
    }
    ...
    if (!isSenderAdmin) {
        return sock.sendMessage(from, { text: '🚫 Only group admins can use `!promote`.' });
    }

    const mentions = msg?.message?.extendedTextMessage?.contextInfo?.mentionedJid;
    if (!mentions || mentions.length === 0) {
        return sock.sendMessage(from, { text: '⚠️ Please tag a user to promote.' });
    }
    ...
    if (targetInfo.admin === 'admin' || targetInfo.admin === 'superadmin') {
        return sock.sendMessage(from, { text: 'ℹ️ This user is already an admin.' });
    }
